...
(() => {
  ...
  const CHIRPCHECK_LEVELS = ["blue", "orange", "red", "gold"];
  ...
  function chirpCheckLevel(p = {}) {
    if (!p) return "";
    const stored = String(p.chirpcheck_level || p.verification_badge || "").toLowerCase();
    ...
    const isOneYearVip = Boolean(p.is_vip) && (
      (Number.isFinite(vipMs) && vipMs > 0 && Date.now() - vipMs >= oneYearMs) ||
      (Number.isFinite(createdMs) && createdMs > 0 && Date.now() - createdMs >= oneYearMs)
    );

    if (p.is_chirp_official || p.is_satv_group || isOneYearVip || stored === "official" || stored === "vip") return "gold";

    const followers = Number(p.followers_count || 0);
    if (followers >= 10000000) return "red";
    if (followers >= 1000000) return "orange";
    if (followers >= 100000 || p.is_verified || p.verification_status === "auto_verified" || p.verification_status === "manual_verified") return "blue";
    return "";
  }

  function chirpCheckLabel(level = "blue") {
    return {
      blue: "ChirpCheck · 100k seguidores",
      orange: "ChirpCheck naranja · 1M seguidores",
      red: "ChirpCheck roja · 10M seguidores",
      gold: "ChirpCheck Gold · VIP u oficial"
    }[level] || "ChirpCheck";
  }
  ...
  async function ensureProfile() {
    ...
    if (!data) {
      const chosenUsername = cleanUsernameInput(user.user_metadata?.username || user.user_metadata?.preferred_username || "");
      const username = validUsername(chosenUsername) ? chosenUsername : `user_${user.id.replaceAll("-", "").slice(0, 24)}`;
  ...
  async function loadMyProfile() {
    $("#profileName") && ($("#profileName").innerHTML = profileNameHTML(profile, "Tu perfil"));
  ...
  async function loadPublicProfile() {
    ...
    const { data: p, error } = await sb.from("profiles").select("*").eq("username", username).maybeSingle();
    if (error) return box.innerHTML = empty("No encontré ese perfil", error.message);
    if (!p) return box.innerHTML = empty("No encontré ese perfil", "Revisá que el usuario exista o abrí el link completo.");
  ...
  function initAuth() {
    ...
    $("#registerForm")?.addEventListener("submit", async e => {
      ...
      try {
        ...
        const username = cleanUsernameInput($("#registerUsername")?.value || "");
        const email = $("#registerEmail").value.trim();
        const password = $("#registerPassword").value;
        if (!validUsername(username)) throw new Error("Usuario: 3 a 30 caracteres, letras, números o guión bajo.");
        ...
        const { data, error } = await wait(sb.auth.signUp({ email, password, options: { emailRedirectTo: CFG.authCallback, data: { display_name: displayName, username } } }), CFG.authTimeoutMs, "Crear cuenta");
        if (error) throw error;
  ...
  async function startDmWithProfile(targetProfile) {
    ...
    $("#dmPeerName") && ($("#dmPeerName").innerHTML = profileNameHTML(targetProfile, "Usuario"));
  ...
  async function fetchProfilesByIds(ids = []) {
    ...
    const { data, error } = await sb.from("profiles").select("*").in("id", uniqueIds);
    if (error) {
