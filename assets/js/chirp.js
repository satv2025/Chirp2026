(() => {
  ...
  function hydrateMe() {
    $$('.js-me-name').forEach(
      (el) => (el.textContent = profile?.display_name || 'Tu perfil')
    );
  ...
  function waitForVideoEvent(video, eventName, timeoutMs = 2500) {
  ...
  async function applyFrame0Poster(video) {
    if (!video || video.dataset.posterReady === '1' || video.poster) return;
    ...
    try {
      ...
      if (video.readyState < 2) {
        video.load();
        await waitForVideoEvent(video, 'loadeddata', 3500);
      }
      ...
      console.warn(
        '[Chirp] No pude crear poster desde el frame 0 del video',
        error
      );
    }
  }
  ...
  async function mediaHtml(rows = []) {
    ...
    for (const m of rows.sort(
      ...
      if (m.media_type === 'video')
        out.push(
          `<div class="chirp-video-wrap"><video class="js-plyr" playsinline preload="auto"><source src="${esc(url)}"></video></div>`
        );
      else
        out.push(
  ...
  async function loadMyProfile() {
    $('#profileName') &&
      ($('#profileName').textContent = profile?.display_name || 'Tu perfil');
    $('#profileUser') &&
  ...
  async function startDmWithProfile(targetProfile) {
    ...
    $('#dmPeerName') &&
      ($('#dmPeerName').textContent = targetProfile?.display_name || 'Usuario');
    $('#dmPeerUser') &&
  ...
  async function findDmUsers(query) {
    ...
    box.innerHTML =
      (data || []).map(dmRow).join('') ||
      `<div class="chip chip-muted">Sin usuarios</div>`;
    bindDmRows(box);
  }
