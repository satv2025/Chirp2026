...
module.exports = async function handler(req, res) {
  try {
    ...
    const preference = await createPreference({
      ...
      back_urls: {
        success: `${base}/gold-return?provider=mercadopago&result=success&order_id=${order.id}`,
        failure: `${base}/gold-return?provider=mercadopago&result=failure&order_id=${order.id}`,
        pending: `${base}/gold-return?provider=mercadopago&result=pending&order_id=${order.id}`,
      },
