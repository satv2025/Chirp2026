...
module.exports = async function handler(req, res) {
  try {
    ...
    if (!amount || amount <= 0) {
      return sendJson(
        res,
        { error: 'El monto del plan de Mercado Pago es inválido.' },
        400
      );
    }
    ...
    const successUrl = `${base}/gold-return?provider=mercadopago&result=success&order_id=${order.id}`;
    const failureUrl = `${base}/gold-return?provider=mercadopago&result=failure&order_id=${order.id}`;
    const pendingUrl = `${base}/gold-return?provider=mercadopago&result=pending&order_id=${order.id}`;

    const payload = {
      items: [
        {
          id: plan.id,
          title: plan.name || 'Chirp Gold',
          description:
            plan.description || 'Suscripción premium de la red social Chirp.',
          quantity: 1,
    ...
    console.log('[Chirp Gold MP create] preference payload:', {
      base,
      order_id: order.id,
    ...
    return sendJson(
      res,
      {
        error: error.message || 'No pude crear el pago de Mercado Pago.',
